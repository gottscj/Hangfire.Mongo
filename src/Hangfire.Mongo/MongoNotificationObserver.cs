...
namespace Hangfire.Mongo
{
    ...
    /// </summary>
    public class MongoNotificationObserver : IBackgroundProcess, IBackgroundProcessAsync, IServerComponent
    {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    using (var cursor = _dbContext.Notifications.FindSync(filter, options, cancellationToken))
                    {
                        ...
                        {
                            lastId = ProcessNotification(doc, lastId);
                        }
                    }
                }
                ...
                catch (Exception e)
                {
                    ...
                    DelaySync(GetFailureTimeoutMs(), cancellationToken);
                }
                finally
                {
        ...
        private ObjectId ProcessNotification(BsonDocument doc, ObjectId lastId)
        {
        ...
        private static void DelaySync(int millisecondsDelay, CancellationToken cancellationToken)
        {
