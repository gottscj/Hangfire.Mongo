...
#pragma warning disable 1591
    [Collection("Database")]
    public class TransactionalMongoWriteOnlyTransactionTests
    {
        ...
        {
            var exception = Assert.Throws<ArgumentNullException>(() => new TransactionalMongoWriteOnlyTransaction(null, new MongoStorageOptions()));
        ...
        private void Commit(Action<TransactionalMongoWriteOnlyTransaction> action)
        {
            using (var transaction = new TransactionalMongoWriteOnlyTransaction(_database, new MongoStorageOptions()))
            {
                action(transaction);
                transaction.Commit();
            }
